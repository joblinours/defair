...
def decrypt_archive(
    archive_path: Path,
    private_keys: List[Union[Path, bytes]],
    output_file: Path,
    unstream_cmd: Path,
    method: str = "auto",
    force: bool = False,
    ...
    logger.debug(
        "Processing archive %s with %d private keys", archive_path, len(private_keys)
    )
    ...
    for private_key in private_keys:
        ...
        if res:
            ...
            if isinstance(private_key, Path):
                log.error("Failed to decrypt archive with private key %s", private_key)
            else:
                log.error("Failed to decrypt archive with this private key")
    return res
...
def decrypt_archive_python(
    ...
    def get_cipher(oid: str, key: bytes, iv: bytes):
        if oid == "2.16.840.1.101.3.4.1.42":  # AES 256 CBC
            ...
            ret = Cipher(
                algorithms.TripleDES(key),  # nosec B304 - Not used for encryption, only decryption
                modes.CBC(iv),
            )
        else:
    ...
    if pkcs7_offset == -1:
        log.error("Invalid PKCS#7 structure: PKCS#7-data OID not found.")
        return False
    elif pkcs7_offset + 2 > len(s):
        log.error(
            "Invalid PKCS#7 structure: PKCS#7-data OID found too far in buffer (%d).",
            pkcs7_offset,
        )
        return False
    oidlen = s[pkcs7_offset + 1]
    if pkcs7_offset + oidlen + 2 > len(s):
        log.error(
            "Invalid PKCS#7 structure: PKCS#7-data OID found too far in buffer (%d).",
            pkcs7_offset,
        )
    ...
    if iv_offset + 2 > len(s):
        log.error("Invalid PKCS#7 structure: IV is too far in buffer (%d).", iv_offset)
        return False
    ivlen = s[iv_offset + 1]
    if iv_offset + ivlen + 2 > len(s):
        log.error("Invalid PKCS#7 structure: IV is too far in buffer (%d).", iv_offset)
        return False
    ...
    if block_size > 256:
        log.error(
            "Invalid cipher block size %d for PKCS#7 (cipher was %s) ",
            block_size,
            cipher,
        )
        return False
    ...
    with open(archive_path, "rb") as f:
        ...
        try:
            ...
            # PKCS#7 will always add padding, even for data that is already a multiple of the cipher block size
            # We need to remove the padding after some sanity checks
            if prev:
                pad_len = prev[-1]
                # Verify that the padding value is valid
                if (
                    1 <= pad_len <= block_size
                    and len(prev) >= pad_len
                    # and that padding bytes are all equal to pad_len
                    and prev[-pad_len:] == bytes([pad_len]) * pad_len
                ):
                    p.stdin.write(prev[:-pad_len])
                else:
                    log.error("Invalid PKCS#7 padding detected in chunk: %s", prev)
                    return False
