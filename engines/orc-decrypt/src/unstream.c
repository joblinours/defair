...
#define MAGIC_EQL(src, dst) (src[0] == dst[0] && src[1] == dst[1] && src[2] == dst[2] && src[3] == dst[3])
...
int do_unstream( FILE *f_in, FILE *f_out )
{
	...
	if ( ret != sizeof(t_jrnl) || !MAGIC_EQL(jrnl.magic, MAGIC_JRNL) )
	{
...
fprintf( stderr, "Unstream: reading stream op at %llx\n", (unsigned long long)ftell( f_in ) );
		...
		if ( MAGIC_EQL(op.magic, MAGIC_CLOSE) )
		{
#ifdef DEBUG
fprintf( stderr, "CLOSE\n" );
		...
		else if ( MAGIC_EQL(op.magic, MAGIC_SEEK) )
		{
			off_t off = op.ull;
...
fprintf( stderr, "SEEK %llx %d\n", (unsigned long long)off, meth );
			...
			// disallow seeks over 100To
#define MAX_SEEK_OFF (100LL*1024*1024*1024*1024)
			if (off > MAX_SEEK_OFF) {
				fprintf( stderr, "Unstream: seek too large\n" );
				return EXIT_FAILURE;
			}
		...
		else if ( MAGIC_EQL(op.magic, MAGIC_WRITE) )
		{
