...
def test_cli_with_input_directories(
    ...
    assert ret == 0, "Unexpected script error"
    assert "0 failed, 4 succeeded" in caplog.records[-1].message, (
...
def test_cli_with_multiple_invalid_keys(
    input_file,
    expected_decrypted_archives,
    expected_hash,
    invalid_recipient_keys,
    ...
    ret = entrypoint(
        [
            ...
            "1",
            "-k",
            str(invalid_recipient_keys[0]),
            str(invalid_recipient_keys[1]),
            "--unstream-path",
            str(unstream_cmd),
            "--log-level",
            "DEBUG",
    ...
    assert (
        ...
        "Decryption process succeeded whereas the key provided were invalid for this archive."
    )
    (tmp_path / input_file.stem).unlink(missing_ok=True)
...
def test_cli_with_one_valid_keys(
    input_file,
    expected_decrypted_archives,
    expected_hash,
    valid_recipient_keys,
    ...
    msg = (
        "Decryption process succeeded whereas it should have failed."
        if expected_decrypted_archives == 0
        else "Failed to decrypt archive."
    )
    ret = entrypoint(
        [
            "-j",
