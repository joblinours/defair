...
use crate::helpers::unified_logger::{EventOrigin, FileFinding, UnifiedLogger, MatchReason, LogLevel};
...
fn scan_memory_buffer(
        ...
        if !sample_matches.is_full() {
            ...
            sample_matches.insert(sample_matches.len(), GenMatch{
                ...
                rule: Some(crate::types::RuleRef {
                    ...
                    name: ymatch.rulename.clone(),
                    namespace: Some(ymatch.namespace.clone()),
                    tags: ymatch.tags.clone(),
                    ..Default::default()
                }),
            });
        }
    ...
    if !sample_matches.is_empty() {
        ...
        logger.file_match(FileFinding {
            ...
            file_created: ctime_str.clone(),
            file_modified: mtime_str.clone(),
            file_accessed: atime_str.clone(),
            // A file on disk carries no record of which machine it was
            // collected from, and no event time of its own, so both stay
            // empty here and consumers fall back to the scan's own values.
...
mod tests {
    ...
    mod program_directory_exclusion_tests {
        ...
        fn test_program_dir_match() {
            let program_dir = "/opt/raijin";
            let file_path = Path::new("/opt/raijin/signatures/rules.yar");
            let program_dir_path = Path::new(program_dir);
        ...
        fn test_program_dir_exact_match() {
            let program_dir = "/opt/raijin";
            let file_path = Path::new("/opt/raijin");
            let program_dir_path = Path::new(program_dir);
            assert!(file_path.starts_with(program_dir_path));
        }
        ...
        fn test_program_dir_no_match_similar_prefix() {
            let program_dir = "/opt/raijin";
            let file_path = Path::new("/opt/raijin2/file.exe");
            let program_dir_path = Path::new(program_dir);
            // This should NOT match because raijin2 is different from raijin
            assert!(!file_path.starts_with(program_dir_path));
        }
        ...
        fn test_program_dir_nested_file() {
            let program_dir = "/usr/local/raijin";
            let file_path = Path::new("/usr/local/raijin/config/excludes.cfg");
            let program_dir_path = Path::new(program_dir);
            assert!(file_path.starts_with(program_dir_path));
        }
    }
